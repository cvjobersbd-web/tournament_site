...
const Login = () => {
  ...
  const handleResetPassword = async (e) => {
    e.preventDefault();
    if (!email) {
      setError('পাসওয়ার্ড রিসেট করতে আপনার ইমেইল দিন!');
      return;
    }
    ...
    try {
      ...
      setError('রিসেট লিংক পাঠানো যায়নি!');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ color: '#fff', fontSize: '28px', fontWeight: 800, marginBottom: '16px' }}>
            {resetSent ? '✅ চেক করুন আপনার ইমেইল' : 'পাসওয়ার্ড রিসেট'}
          </h2>
          
          {resetSent ? (
            <>
              <p style={{ color: '#4caf50', marginBottom: '24px' }}>
                আমরা {email} এ পাসওয়ার্ড রিসেট লিংক পাঠিয়েছি!
              </p>
              <button
                onClick={() => {
                  setResetMode(false);
            ...
            <>
              ...
              />
              {error && <p style={{ color: '#ff5722', marginBottom: '16px', fontSize: '13px' }}>{error}</p>}
              <button
                onClick={handleResetPassword}
                disabled={loading}
                style={{
                  width: '100%',
                  background: loading ? '#555' : 'linear-gradient(90deg, #ff9800, #ff5722)',
                  color: '#fff',
                  border: 'none',
                  borderRadius: '30px',
                  padding: '14px',
              ...
              >
                {loading ? 'সেন্ড হচ্ছে...' : 'রিসেট লিংক পাঠান'}
              </button>
              <button
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px', flexWrap: 'wrap', gap: '12px' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', color: 'rgba(255,255,255,0.8)', fontSize: '13px' }}>
              <input
                type="checkbox"
                checked={rememberMe}
                onChange={(e) => setRememberMe(e.target.checked)}
                style={{ width: '16px', height: '16px', cursor: 'pointer', accentColor: '#ff9800' }}
              />
              Remember Me
            </label>
            <button
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <Link to="/signup" style={{ color: '#ff9800', fontSize: '14px', textDecoration: 'none', fontWeight: 700 }}>
              Sign Up
            </Link>
          </div>
        </form>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
          ...
          >
            ...
            <span style={{ color: '#fff', fontSize: '13px', fontWeight: 500 }}>Continue with Google</span>
          </button>
        </div>
      </div>
    </div>
